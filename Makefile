...
lint-js:
	BABEL_ENV=test $(YARN) eslint scripts $(SOURCES) '*.{js,ts}' --format=codeframe --ext .js,.jsx,.cjs,.mjs,.ts,.tsx
...
prepublish:
	...
	rm -rf packages/*/lib/**/*.d.ts{,.map}
	rm -rf codemods/*/lib/**/*.d.ts{,.map}
	rm -rf eslint/*/lib/**/*.d.ts{,.map}
	$(MAKE) clean-tsconfig
	# $(MAKE) build-typescript-typings
